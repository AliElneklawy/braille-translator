...
class ProcessImage:
    ...
    def filter_BW_image(self):  # BW means Black and white
        ...
        self.filtered_image = cv2.morphologyEx(
            self.blacknwhite_image, cv2.MORPH_OPEN, kernel
        )
    ...
    def convert_image_to_2D_array(
        self,
    ):  # assiging pixel values to the arrays 1 if black 0 if white
        self.pixelvaluearray = [
    ...
    def get_sum_of_rows_columns_black_pixels(self):
        ...
        self.column_sums = [
            sum(column) for column in transposed_array
        ]  # calculating sum of each column
        self.row_sums = [
            sum(row) for row in self.pixelvaluearray
    ...
    def shortest_sequence_of_zeros(
        self, arr
    ):  # finds the length of the shortest subsequence
        ...
        for pixel in arr:
            if pixel == 0:
                ...
                if (
                    current_sequence
                ):  # Update min_length only if a sequence of zeros was found
    ...
    def determine_distances_within_symbols(
        self,
    ):  # constant margins surronding the symbol
        shortest_sequence_length_column = self.shortest_sequence_of_zeros(
            self.column_sums
        )
    ...
    def extract_elements(
        self, array, last_elements
    ):  # updating the coordinates for symbols and not just columns and rows
        new_array = []
    ...
    def assign_coordinates_of_elements_in_two_lists(self):
        self.symbol_column_coordinates = self.extract_elements(
            self.column_coordinates, 4
        )
    ...
    def crop_the_image_into_symbols(self):
        ...
        for i in range(half_row_length):
            for j in range(half_column_length):
                cropped_image = self.paper_image.crop(
                    box=(
                        self.symbol_column_coordinates[j * 2],
                        self.symbol_row_coordinates[i * 2],
                        self.symbol_column_coordinates[(j * 2) + 1] + 2,
                        self.symbol_row_coordinates[i * 2 + 1] + 2,
                    )
                )
