...
async def start(update: Update, context: CallbackContext):
    ...
    await update.message.reply_text(
        f"Hey {first_name}. Please, type /standards to get information about the input image stadards."
    )
...
async def repo(update: Update, context: CallbackContext):
    await update.message.reply_text(
        "You can find the code to the project on our GitHub: \
...
async def start_translation(
    update: Update, context: CallbackContext, oProcessImage, oInference, oCorrect
):
    ...
    try:
        for im in tg_tqdm(
            images_arr_preprocessed, TOKEN, chat_id, unit=" chars"
        ):  # show progress bar
            encoded_arr.append(oInference.predict(im))
    except TelegramError as e:
        print(
            "TelegramError occurred. Progress message in tg_tqdm encountered an error:",
            e,
        )
    except ProtocolError:  # TO BE RECHECKED WITH LINE 178 (WORKS)
        await update.message.reply_text(
            "Something went wrong. Please, resend the image."
        )
        return ""
...
async def generate_voice(
    update: Update, context: CallbackContext, current_dir, oVoice, text
):
...
def create_speech_to_braille(oVoiceToText, oTextToBraille, current_dir):
    async def speech_to_braille(update: Update, context: CallbackContext):
        ...
        message = await update.message.reply_text(
            "Transcribtion and translation in progress...."
        )
...
def main():
    ...
    model_path = os.path.join(
        os.path.dirname(current_dir), "models", "grade_1_model.h5"
    )
    ...
    handle_photo_handler = create_handle_photo(
        oVoice, oCorrect, oInference, current_dir
    )
    speech_to_braille_handler = create_speech_to_braille(
        oVoiceToText, oTextToBraille, current_dir
    )
    ...
    # message handlers
    app.add_handler(
        MessageHandler(filters.PHOTO, handle_photo_handler, block=False)
    )  # handle photo
    app.add_handler(
        MessageHandler(filters.TEXT & ~filters.COMMAND, text_to_braille_handler)
    )  # handle text
    app.add_handler(
        MessageHandler(
            filters.VOICE | filters.AUDIO | filters.VIDEO, speech_to_braille_handler
        )
    )  # handle voice, video and audio
